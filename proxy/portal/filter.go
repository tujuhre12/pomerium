...
// UserInfo is the computed user information needed for access decisions.
...
func CheckRouteAccess(user UserInfo, route *config.Policy) bool {
	ppl := route.ToPPL()
	return checkPPLAccess(user, ppl)
}

func checkPPLAccess(user UserInfo, ppl *parser.Policy) bool {
...
func checkPPLCriterionAccess(user UserInfo, criterion parser.Criterion) bool {
